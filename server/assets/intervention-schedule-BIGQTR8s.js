function interventionFrequencyMinutes(frequency) {
...
function endOfCurrentShift(now) {
...
function scheduledInterventions(interventions, logs, problems, now) {
  const nowMs = now.getTime();
  const problemById = new Map(problems.map((problem) => [problem.id, problem]));
  const logsByIntervention = /* @__PURE__ */ new Map();
  ...
  return interventions.map((intervention) => {
    const interventionLogs = (logsByIntervention.get(intervention.id) || []).sort(
      (a, b) => `${b.date}T${b.time || "00:00"}`.localeCompare(`${a.date}T${a.time || "00:00"}`)
    );
    const completion = interventionLogs[0];
    const intervalMinutes = interventionFrequencyMinutes(intervention.frequencyType);
    const baseTimestamp = completion ? (/* @__PURE__ */ new Date(`${completion.date}T${completion.time || "08:00"}`)).getTime() : (/* @__PURE__ */ new Date(`${intervention.startDate}T08:00`)).getTime();
    const dueAt = Number.isFinite(baseTimestamp) ? new Date(baseTimestamp + (intervalMinutes ?? 0) * 6e4) : null;
    let status = "upcoming";
    if (["cancelled", "discontinued", "superseded"].includes(intervention.status)) {
      ...
      const minutesUntilDue = Math.floor((dueAt.getTime() - nowMs) / 6e4);
    ...
    return rank[a.status] - rank[b.status] || (a.dueAt?.getTime() ?? Number.MAX_SAFE_INTEGER) - (b.dueAt?.getTime() ?? Number.MAX_SAFE_INTEGER);
  });
}
function scheduledInterventionLabel(status) {
...
export {
  scheduledInterventionLabel as a,
  endOfCurrentShift as e,
  scheduledInterventions as s
};
