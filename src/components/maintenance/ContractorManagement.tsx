...
export function ContractorManagement({ initialTab = "overview", mode = "list", contractorId }: { initialTab?: Tab; mode?: Mode; contractorId?: string }) {
  ...
  if (!care.canAccess("permission.manage")) return <div className="p-6"><Card><CardContent className="py-10 text-center text-sm text-muted-foreground">You do not have permission to view Contractors.</CardContent></Card></div>;
  ...
  return (
    <div className="space-y-5 p-4 md:p-6">
      ...
      <div className="flex flex-wrap gap-2"><Button onClick={() => navigate({ to: "/maintenance/contractors/new" })}><Plus className="mr-2 h-4 w-4" />New Contractor</Button><Button variant="outline" onClick={() => navigate({ to: "/maintenance/contractors/register" })}>Open Register</Button></div>
...
function ContractorDetail({ contractor, message, setMessage, run }: { contractor: MaintenanceContractor; message: string; setMessage: (message: string) => void; run: (action: () => void, success: string) => void }) {
  ...
  const notes = care.maintenanceContractorNotes.filter((item) => item.contractorId === contractor.id);
  ...
  return (
    <div className="space-y-5 p-4 md:p-6">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">{PROFILE_TABS.map((item) => <button key={item.value} type="button" onClick={() => setTab(item.value)} className={cn("shrink-0 rounded-md border px-3 py-2 text-sm", tab === item.value ? "border-primary bg-primary text-primary-foreground" : "bg-background hover:bg-muted")}>{item.label}</button>)}</div>
...
function NotesPanel({ rows, onNew, onEdit, run }: { rows: MaintenanceContractorNote[]; onNew: () => void; onEdit: (row: MaintenanceContractorNote) => void; run: (a: () => void, s: string) => void }) {
  const care = useCare();
  return <Card><CardHeader><div className="flex items-center justify-between gap-3"><CardTitle>Notes</CardTitle><Button onClick={onNew}><Plus className="mr-2 h-4 w-4" />Add Note</Button></div></CardHeader><CardContent>{rows.length ? <div className="space-y-2">{rows.map((row) => <div key={row.id} className={cn("rounded-md border p-3 text-sm", !row.active && "opacity-60")}><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="font-semibold">{row.pinned ? "Pinned - " : ""}{row.title}</div><div className="text-xs text-muted-foreground">{label(row.noteType)} - {row.homeId ? homeName(care, row.homeId) : "Company-wide"} - {formatDateTime(row.createdAt)}</div></div><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => onEdit(row)}>Edit</Button><Button size="sm" variant="outline" onClick={() => run(() => care.pinMaintenanceContractorNote(row.id, !row.pinned), "Pinned state updated.")}>{row.pinned ? "Unpin" : "Pin"}</Button>{row.active ? <Button size="sm" variant="outline" onClick={() => run(() => care.removeMaintenanceContractorNote(row.id, promptReason("Remove note reason") || ""), "Note removed.")}>Remove</Button> : <Button size="sm" variant="outline" onClick={() => run(() => care.restoreMaintenanceContractorNote(row.id), "Note restored.")}>Restore</Button>}</div></div><p className="mt-2 text-muted-foreground">{row.body}</p></div>)}</div> : <Empty text="No internal notes have been recorded." />}</CardContent></Card>;
}
...
function HomeDialog({ open, association, contractor, onOpenChange }: { open: boolean; association?: MaintenanceContractorHomeAssociation; contractor: MaintenanceContractor; onOpenChange: (open: boolean) => void }) {
  ...
  const [message, setMessage] = useState("");
  const update = (key: keyof typeof form, value: any) => setForm((state) => ({ ...state, [key]: value }));
  function submit() { try { association ? care.updateMaintenanceContractorHomeAssociation(association.id, form) : care.associateMaintenanceContractorHome(contractor.id, form); onOpenChange(false); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to save Home access."); } }
...
function NoteDialog({ open, note, contractor, onOpenChange }: { open: boolean; note?: MaintenanceContractorNote; contractor: MaintenanceContractor; onOpenChange: (open: boolean) => void }) {
  ...
  const [message, setMessage] = useState("");
  const update = (key: keyof typeof form, value: any) => setForm((state) => ({ ...state, [key]: value }));
  function submit() { try { note ? care.updateMaintenanceContractorNote(note.id, form) : care.addMaintenanceContractorNote(contractor.id, form); onOpenChange(false); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to save note."); } }
