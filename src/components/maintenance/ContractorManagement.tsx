...
import { appendComplianceReview, contractorReviewPresentation, loadPhase5AGovernance, persistPhase5AGovernance, type ContractorComplianceReview } from "@/domain/maintenance/phase5aGovernance";
...
export function ContractorManagement({ initialTab = "overview", mode = "list", contractorId, embedded = false }: { initialTab?: Tab; mode?: Mode; contractorId?: string; embedded?: boolean }) {
  const care = useCare();
  ...
  return (
    <div className={cn("space-y-5", !embedded && "p-4 md:p-6")}>
      {!embedded && <Header title="Contractors" subtitle="Operational contractor register, contacts, Home access and service coverage." current="Contractors" />}
      {message && <Notice>{message}</Notice>}
      <div className="flex flex-wrap gap-2">{care.canAccess("maintenance.contractors.create") && <Button onClick={() => embedded ? setNewContractorOpen(true) : navigate({ to: "/maintenance/contractors/new" })}><Plus className="mr-2 h-4 w-4" />New Contractor</Button>}{!embedded && <Button variant="outline" onClick={() => navigate({ to: "/maintenance/contractors/register" })}>Open Register</Button>}</div>
      {!embedded && <div className="flex gap-2 overflow-x-auto pb-1">{TABS.map((item) => <Link key={item.value} to={item.to} onClick={() => { setTab(item.value); setPage(1); }} className={cn("shrink-0 rounded-md border px-3 py-2 text-sm", tab === item.value ? "border-primary bg-primary text-primary-foreground" : "bg-background hover:bg-muted")}>{item.label}</Link>)}</div>}
      ...
      <Dialog open={newContractorOpen} onOpenChange={setNewContractorOpen}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl"><DialogHeader><DialogTitle>New Contractor</DialogTitle></DialogHeader><ContractorForm onCancel={() => setNewContractorOpen(false)} onDone={() => { setNewContractorOpen(false); setMessage("Contractor created successfully."); }} /></DialogContent></Dialog>
    </div>
  );
}
...
function ContractorDetail({ contractor, message, setMessage, run }: { contractor: MaintenanceContractor; message: string; setMessage: (message: string) => void; run: (action: () => void, success: string) => void }) {
  ...
  const [governance, setGovernance] = useState(loadPhase5AGovernance);
  const [reviewOpen, setReviewOpen] = useState(false);
  const [review, setReview] = useState({ decision: "APPROVED" as ContractorComplianceReview["decision"], conditions: "", nextReviewDate: "", comments: "", missingRequirements: "", nursingHomeId: "" });
  ...
  const reviews = governance.reviews.filter((item) => item.contractorId === contractor.id).sort((a, b) => b.reviewedAt.localeCompare(a.reviewedAt));
  ...
  return (
    <div className="space-y-5 p-4 md:p-6">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {!contractor.archived && care.canAccess("permission.manage") && <Button variant="outline" onClick={() => setReviewOpen(true)}>Start Compliance Review</Button>}
      ...
      {tab === "overview" && <Card><CardHeader><CardTitle>Compliance Review History</CardTitle></CardHeader><CardContent>{reviews.length ? <div className="space-y-3">{reviews.map((item, index) => <div key={item.id} className={cn("rounded-lg border p-4", index === 0 && "border-primary bg-primary/5")}><div className="flex flex-wrap items-center justify-between gap-2"><div className="font-semibold">{contractorReviewPresentation(item)}</div>{index === 0 && <Badge>Latest review</Badge>}</div><div className="mt-1 text-sm text-muted-foreground">{formatDateTime(item.reviewedAt)} · {item.reviewScope === "GLOBAL" ? "Global" : homeName(care, item.nursingHomeId || "")} · {item.reviewedByNameSnapshot}</div>{item.conditions && <div className="mt-2 text-sm"><strong>Conditions:</strong> {item.conditions}</div>}{item.missingRequirements.length > 0 && <div className="mt-1 text-sm"><strong>Missing:</strong> {item.missingRequirements.join(", ")}</div>}<div className="mt-1 text-sm">Next review: {item.nextReviewDate || "Not scheduled"}</div></div>)}</div> : <Empty text="No compliance reviews have been recorded." />}</CardContent></Card>}
      ...
      <Dialog open={reviewOpen} onOpenChange={setReviewOpen}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>Contractor Compliance Review</DialogTitle></DialogHeader><div className="space-y-4"><Notice>Current status: {contractor.approvalStatus.replaceAll("_", " ")} · {associations.filter((item) => item.active).length} active Home records · {serviceAreas.filter((item) => item.active).length} services</Notice><Select label="Review Scope" value={review.nursingHomeId} onChange={(value) => setReview({ ...review, nursingHomeId: value })} options={[{ value: "", label: "Global review" }, ...associations.filter((item) => item.active).map((item) => ({ value: item.homeId, label: homeName(care, item.homeId) }))]} /><Select label="Decision" value={review.decision} onChange={(value) => setReview({ ...review, decision: value as any })} options={["APPROVED","APPROVED_WITH_CONDITIONS","MORE_INFORMATION_REQUIRED","RESTRICTED","SUSPENDED","REJECTED"].map((value) => ({ value, label: label(value) }))} />{review.decision === "APPROVED_WITH_CONDITIONS" && <><label className="text-sm font-medium">Conditions</label><Textarea value={review.conditions} onChange={(event) => setReview({ ...review, conditions: event.target.value })} /><Field label="Next Review Date" type="date" value={review.nextReviewDate} onChange={(value) => setReview({ ...review, nextReviewDate: value })} /></>}<label className="text-sm font-medium">Missing requirements or information requested</label><Textarea value={review.missingRequirements} onChange={(event) => setReview({ ...review, missingRequirements: event.target.value })} /><label className="text-sm font-medium">Review notes</label><Textarea value={review.comments} onChange={(event) => setReview({ ...review, comments: event.target.value })} /></div><DialogFooter><Button variant="outline" onClick={() => setReviewOpen(false)}>Cancel</Button><Button onClick={() => { try { const now = new Date().toISOString(); const item: ContractorComplianceReview = { id: `contractor-review-${contractor.id}-${Date.now()}`, tenantId: contractor.tenantId, contractorId: contractor.id, nursingHomeId: review.nursingHomeId || undefined, reviewScope: review.nursingHomeId ? "NURSING_HOME" : "GLOBAL", reviewedByUserId: care.currentUser.id, reviewedByNameSnapshot: care.currentUser.name, reviewedAt: now, decision: review.decision, complianceStatusSnapshot: contractor.approvalStatus, serviceIdsReviewed: serviceAreas.filter((service) => service.active).map((service) => service.id), insuranceFindings: [], certificateFindings: [], missingRequirements: review.missingRequirements.split(",").map((value) => value.trim()).filter(Boolean), conditions: review.conditions || undefined, nextReviewDate: review.nextReviewDate || undefined, comments: review.comments || undefined, evidenceReferences: [], createdAt: now, updatedAt: now }; const next = { ...governance, reviews: appendComplianceReview(governance.reviews, item) }; persistPhase5AGovernance(next); setGovernance(next); setReviewOpen(false); setMessage("Compliance review saved without replacing previous reviews."); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to save review."); } }}>Complete Review</Button></DialogFooter></DialogContent></Dialog>
    </div>
  );
}
...
function ContractorForm({ contractor, onDone, onCancel }: { contractor?: MaintenanceContractor; onDone: (id: string) => void; onCancel?: () => void }) {
  const care = useCare();
  const [message, setMessage] = useState("");
