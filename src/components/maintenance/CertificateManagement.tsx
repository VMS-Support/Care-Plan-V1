...
export function CertificateManagement({ initialTab = "overview", mode = "list", certificateId, embedded = false }: { initialTab?: Tab; mode?: Mode; certificateId?: string; embedded?: boolean }) {
  const care = useCare();
  ...
  return (
    <div className={cn("space-y-5", !embedded && "p-4 md:p-6")}>
      {!embedded && <Header title="Certificates" subtitle="Shared certificate register, renewals, expiry monitoring and missing compliance requirements." />}
      {message && <div className="rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-blue-900">{message}</div>}
      <div className="flex flex-wrap gap-2">
        <Button onClick={() => embedded ? setCertificateDialog({ open: true, action: "new" }) : navigate({ to: "/maintenance/certificates/new" })}><Plus className="mr-2 h-4 w-4" />Create Certificate</Button>
        <Button variant="outline" onClick={() => embedded ? setTab("types") : navigate({ to: "/maintenance/certificates/types" })}>Manage Certificate Types</Button>
        <Button variant="outline" onClick={() => embedded ? setTab("requirements") : navigate({ to: "/maintenance/certificates/requirements" })}>Manage Requirements</Button>
        {tab === "types" && <Button onClick={() => setTypeDialog({ open: true })}><Plus className="mr-2 h-4 w-4" />Add Certificate Type</Button>}
        {tab === "requirements" && <Button onClick={() => setRequirementDialog({ open: true })}><Plus className="mr-2 h-4 w-4" />Add Requirement</Button>}
      </div>
      {!embedded && <div className="flex gap-2 overflow-x-auto pb-1">
...
function CertificateDetail({ certificate }: { certificate: MaintenanceCertificate }) {
  const care = useCare();
  const [verification, setVerification] = useState<{ open: boolean; result: "APPROVED" | "REJECTED" | "MORE_INFORMATION_REQUIRED"; comment: string; reason: string }>({ open: false, result: "APPROVED", comment: "", reason: "" });
  ...
  const current = versions.find((item) => item.id === certificate.currentVersionId);
  const submitVerification = () => { if (!current) return; care.verifyMaintenanceCertificateVersion(certificate.id, current.id, verification.result, verification.comment, verification.reason); setVerification((state) => ({ ...state, open: false })); };
  return <><div className="grid gap-4 xl:grid-cols-[1fr_0.85fr]"><Card><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><CardTitle>{certificate.title}</CardTitle>{current && care.canAccess("maintenance.certificates.verify") && <Button onClick={() => setVerification((state) => ({ ...state, open: true }))}>Verify Certificate</Button>}</div></CardHeader><CardContent className="space-y-4"><div className="grid gap-3 sm:grid-cols-3"><Stat label="Compliance" value={certificateComplianceStatus({ certificate, version: current, type, attachments: attachments.filter((item) => item.certificateVersionId === certificate.currentVersionId) }).replaceAll("_", " ")} /><Stat label="Current Version" value={`v${current?.versionNumber || 0}`} /><Stat label="Verification" value={current?.verificationStatus?.replaceAll("_", " ") || "Not required"} /></div><Info title="Certificate Number" value={certificate.certificateNumber} /><Info title="Type" value={type?.name || "Unknown"} /><Info title="Issuing Organisation" value={certificate.issuingOrganisation} /><Info title="Subject" value={subjectLabel(certificate, care.maintenanceAssets, care.maintenanceWorkOrders, care.safetyInspections)} /><InfoList title="Attachments" rows={attachments.filter((item) => item.active && !item.removedAt).map((item) => `${item.primaryAttachment ? "Primary - " : ""}${item.fileName} (${item.documentType.replaceAll("_", " ")})`)} empty="No attachments have been uploaded for this certificate version." /></CardContent></Card><Card><CardHeader><CardTitle>Version History & Timeline</CardTitle></CardHeader><CardContent className="space-y-4"><div className="space-y-2">{versions.map((version) => <div key={version.id} className="rounded-md border p-3 text-sm"><div className="flex items-center justify-between gap-2"><div className="font-medium">Version {version.versionNumber}{version.isCurrent ? " · Current" : " · Superseded"}</div><Badge className={statusClass(versionPresentationStatus(version, type))}>{versionPresentationStatus(version, type).replaceAll("_", " ")}</Badge></div><div className="text-xs text-muted-foreground">Issued {version.issuedDate} · Valid from {version.validFromDate} · Expires {version.expiryDate || "No expiry"}</div><div className="mt-1 text-xs">Verification: {version.verificationStatus?.replaceAll("_", " ") || "Not required"}{version.verifiedBy ? ` by ${version.verifiedBy}` : ""}</div>{version.renewalReason && <div className="mt-1 text-xs">{version.renewalReason}</div>}</div>)}</div><MaintenanceTimeline events={timeline.map((item, index) => ({ id: `${certificate.id}-${item.at}-${index}`, at: item.at, title: item.summary, actor: item.user, reference: item.reference, category: "Certificate" }))} empty="No certificate activity has been recorded." /></CardContent></Card></div><Dialog open={verification.open} onOpenChange={(open) => setVerification((state) => ({ ...state, open }))}><DialogContent><DialogHeader><DialogTitle>Verify Certificate</DialogTitle><DialogDescription>Record an auditable decision for version {current?.versionNumber}.</DialogDescription></DialogHeader><div className="space-y-3"><Select label="Result" value={verification.result} onChange={(result) => setVerification((state) => ({ ...state, result: result as any }))} options={[{ value: "APPROVED", label: "Approved" }, { value: "REJECTED", label: "Rejected" }, { value: "MORE_INFORMATION_REQUIRED", label: "More Information Required" }]} /><label className="block text-sm font-medium">Comment</label><Textarea value={verification.comment} onChange={(event) => setVerification((state) => ({ ...state, comment: event.target.value }))} />{verification.result === "REJECTED" && <><label className="block text-sm font-medium">Rejection reason</label><Textarea value={verification.reason} onChange={(event) => setVerification((state) => ({ ...state, reason: event.target.value }))} /></>}</div><DialogFooter><Button variant="outline" onClick={() => setVerification((state) => ({ ...state, open: false }))}>Cancel</Button><Button onClick={submitVerification}>Save verification</Button></DialogFooter></DialogContent></Dialog></>;
}
