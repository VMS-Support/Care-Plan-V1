...
function LinkDialog({ open, certificate, onOpenChange }: { open: boolean; certificate?: MaintenanceCertificate; onOpenChange: (v: boolean) => void }) {
  ...
  const [message, setMessage] = useState("");
  const options = kind === "asset" ? care.maintenanceAssets.map((item) => ({ value: item.id, label: item.assetName })) : kind === "workOrder" ? care.maintenanceWorkOrders.map((item) => ({ value: item.id, label: item.workOrderNumber })) : care.safetyInspections.map((item) => ({ value: item.id, label: item.inspectionNumber }));
  function submit() { try { if (!certificate) throw new Error("Certificate not selected."); if (kind === "asset") care.linkMaintenanceCertificateAsset(certificate.id, targetId, "APPLIES_TO"); else if (kind === "workOrder") care.linkMaintenanceCertificateWorkOrder(certificate.id, targetId, "RELATED_TO"); else care.linkMaintenanceCertificateSafetyInspection(certificate.id, targetId, "SUPPORTS_COMPLIANCE"); onOpenChange(false); } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to link certificate."); } }
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>Link Certificate</DialogTitle></DialogHeader>{message && <div className="rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-900">{message}</div>}<Select label="Link Type" value={kind} onChange={(v) => { setKind(v); setTargetId(""); }} options={[{ value: "asset", label: "Asset" }, { value: "workOrder", label: "Work Order" }, { value: "inspection", label: "Safety Inspection" }]} /><Select label="Target" value={targetId} onChange={setTargetId} options={[{ value: "", label: "Select target" }, ...options]} /><DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button><Button onClick={submit}>Link</Button></DialogFooter></DialogContent></Dialog>;
}
