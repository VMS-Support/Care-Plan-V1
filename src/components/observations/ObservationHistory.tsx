...
export function ObservationHistory({ residentId }: { residentId: string }) {
  const { vitals, residents, currentRole, operationalContext } = useCare();
  ...
  return <div className="space-y-4">
    ...
    <Card><CardHeader className="pb-2"><CardTitle className="text-sm flex items-center gap-2"><Activity className="h-4 w-4" /> {chartType.replaceAll("_", " ")} trend</CardTitle></CardHeader><CardContent>{chart.length < 2 ? <p className="text-sm text-muted-foreground">Not enough observations are available to show a trend.</p> : <div className="flex items-end gap-1 h-28" aria-label={`${chartType} trend`}>{chart.map((point) => { const values = chart.map((item) => item.value); const min = Math.min(...values); const max = Math.max(...values); const height = max === min ? 50 : 20 + ((point.value - min) / (max - min)) * 75; return <div key={point.id} className="flex-1 min-w-2 rounded-t bg-primary/70" style={{ height: `${height}%` }} title={`${point.value} · ${formatDate(point.observedAt)}`} />; })}</div>}</CardContent></Card>
