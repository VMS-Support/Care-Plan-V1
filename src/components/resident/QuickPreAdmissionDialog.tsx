...
export function QuickPreAdmissionDialog({
  open,
  onOpenChange,
}: {
  open: boolean;
  ...
  const update = (patch: Partial<typeof blank>) => setForm((current) => ({ ...current, ...patch }));
  const close = () => {
    setForm(blank);
    onOpenChange(false);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={(value) => value || close()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Create Pre-Admission</DialogTitle>
        </DialogHeader>
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <Choice
            ...
            options={referredFrom}
            onChange={(value) => update({ referredFrom: value })}
          />
