...
function ProbationTable({ title: tableTitle, rows, empty, action }: { title: string; rows: StaffProbation[]; empty: string; action?: (probation: StaffProbation) => ReactNode }) {
  const care = useCare();
  return <Card><CardHeader><CardTitle>{tableTitle}</CardTitle></CardHeader><CardContent><div className="overflow-hidden rounded-lg border"><table className="w-full text-left text-sm"><thead className="bg-muted/50 text-xs text-muted-foreground"><tr><th className="px-4 py-3">Staff</th><th className="px-4 py-3">Dates</th><th className="px-4 py-3">Status</th><th className="px-4 py-3"></th></tr></thead><tbody>{rows.map((row) => <tr key={row.id} className="border-t"><td className="px-4 py-3 font-medium">{staffName(care, row.staffMemberId)}</td><td className="px-4 py-3 text-muted-foreground">{row.probationStartDate} to {row.currentExpectedEndDate}</td><td className="px-4 py-3"><Badge variant="outline">{title(row.status)}</Badge></td><td className="px-4 py-3 text-right">{action?.(row)}</td></tr>)}{rows.length === 0 && <tr><td colSpan={4} className="px-4 py-10 text-center text-muted-foreground">{empty}</td></tr>}</tbody></table></div></CardContent></Card>;
}
...
function ReviewTable({ title: tableTitle, rows, empty, action }: { title: string; rows: StaffProbationReview[]; empty: string; action?: (review: StaffProbationReview) => ReactNode }) {
  const care = useCare();
  return <Card><CardHeader><CardTitle>{tableTitle}</CardTitle></CardHeader><CardContent><div className="overflow-hidden rounded-lg border"><table className="w-full text-left text-sm"><thead className="bg-muted/50 text-xs text-muted-foreground"><tr><th className="px-4 py-3">Staff</th><th className="px-4 py-3">Review</th><th className="px-4 py-3">Scheduled</th><th className="px-4 py-3">Status</th><th className="px-4 py-3"></th></tr></thead><tbody>{rows.map((row) => <tr key={row.id} className="border-t"><td className="px-4 py-3 font-medium">{staffName(care, row.staffMemberId)}</td><td className="px-4 py-3">Review {row.reviewNumber}</td><td className="px-4 py-3 text-muted-foreground">{row.scheduledDate}</td><td className="px-4 py-3"><Badge variant="outline">{title(row.status)}</Badge></td><td className="px-4 py-3 text-right">{action?.(row)}</td></tr>)}{rows.length === 0 && <tr><td colSpan={5} className="px-4 py-10 text-center text-muted-foreground">{empty}</td></tr>}</tbody></table></div></CardContent></Card>;
}
