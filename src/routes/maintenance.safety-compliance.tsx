...
function SafetyComplianceRoute() {
  ...
  return (
    <div className="space-y-5 p-4 md:p-6">
      ...
      {tab === "inspections" && <InspectionsPanel inspections={filteredInspections} categories={categories} assets={care.maintenanceAssets} selected={selectedInspection} onOpen={setSelectedInspectionId} onEvidence={(inspection) => setEvidenceDialog({ open: true, inspection })} onObservation={(inspection) => setObservationDialog({ open: true, inspection })} onComplete={(id) => run(() => care.completeSafetyInspection(id, { declarationAccepted: true, summary: "Inspection completed from Safety & Compliance workspace." }), "Inspection completed.")} onVerify={(id) => run(() => care.verifySafetyInspection(id, "Reviewed in Safety & Compliance workspace."), "Inspection verified.")} onReject={(id) => run(() => care.rejectSafetyInspection(id, { reasonCode: "OTHER", details: "Returned for further detail.", furtherWorkRequired: "Address the verification findings.", newDueDate: addDays(7), reinspectionRequired: true, restrictionRemainsActive: true, additionalEvidenceRequired: true }), "Inspection rejected and further work recorded.")} onCorrective={(id) => run(() => care.createSafetyCorrectiveWorkOrder(id), "Corrective Work Order created or opened.")} onCorrectiveAction={(id, responseId) => run(() => care.createSafetyCorrectiveAction(id, responseId, { dueDate: addDays(7) }), "Corrective Action created or opened.")} onReinspection={(id) => run(() => { const next = care.createSafetyReinspection(id, { dueDate: addDays(7) }); setSelectedInspectionId(next.id); }, "Reinspection created as a separate record.")} />}
...
function InspectionsPanel(props: { inspections: SafetyInspection[]; categories: SafetyCategory[]; assets: MaintenanceAsset[]; selected?: SafetyInspection; onOpen: (id: string) => void; onEvidence: (inspection: SafetyInspection) => void; onObservation: (inspection: SafetyInspection) => void; onComplete: (id: string) => void; onVerify: (id: string) => void; onReject: (id: string) => void; onCorrective: (id: string) => void; onCorrectiveAction: (id: string, responseId: string) => void; onReinspection: (id: string) => void }) {
  const care = useCare();
  return <div className="grid gap-4 xl:grid-cols-[1fr_0.9fr]"><Card><CardHeader><CardTitle>Inspection History</CardTitle></CardHeader><CardContent>{props.inspections.length === 0 ? <Empty text="No inspections match the selected filters." /> : <table className="w-full text-sm"><thead className="border-b bg-muted/40 text-left text-xs uppercase text-muted-foreground"><tr><th className="px-3 py-2">Inspection</th><th className="px-3 py-2">Type</th><th className="px-3 py-2">Subject</th><th className="px-3 py-2">Result</th><th className="px-3 py-2">Verification</th><th className="px-3 py-2">Follow-up</th><th className="px-3 py-2">Action</th></tr></thead><tbody>{props.inspections.map((inspection) => <tr key={inspection.id} className="border-b last:border-0"><td className="px-3 py-3"><button className="font-medium text-blue-700 hover:underline" onClick={() => props.onOpen(inspection.id)}>{inspection.inspectionNumber}</button><div className="text-xs text-muted-foreground">{inspection.inspectionDate}</div></td><td className="px-3 py-3">{inspection.inspectionType.replaceAll("_", " ")}</td><td className="px-3 py-3">{assetName(inspection.assetId, props.assets) || inspection.locationId || "Home-level"}</td><td className="px-3 py-3"><Badge className={resultClass(inspection.overallResult)}>{inspection.overallResult.replaceAll("_", " ")}</Badge></td><td className="px-3 py-3">{inspection.verificationStatus.replaceAll("_", " ")}</td><td className="px-3 py-3 text-xs">{inspection.linkedWorkOrderIds?.length || (inspection.correctiveWorkOrderId ? 1 : 0)} WO · {inspection.linkedCorrectiveActionIds?.length || 0} CA</td><td className="px-3 py-3"><Button size="sm" variant="outline" onClick={() => props.onOpen(inspection.id)}>Open</Button></td></tr>)}</tbody></table>}</CardContent></Card>{props.selected ? <InspectionDetail inspection={props.selected} categories={props.categories} assets={props.assets} responses={care.safetyInspectionResponses.filter((item) => item.inspectionId === props.selected!.id)} observations={care.safetyInspectionObservations.filter((item) => item.inspectionId === props.selected!.id)} evidence={care.safetyInspectionEvidence.filter((item) => item.inspectionId === props.selected!.id)} certificates={care.safetyCertificates.filter((item) => item.inspectionId === props.selected!.id)} requirements={care.safetyInspectionTemplateEvidenceRequirements.filter((item) => item.templateId === props.selected!.templateId)} onEvidence={() => props.onEvidence(props.selected!)} onObservation={() => props.onObservation(props.selected!)} onComplete={() => props.onComplete(props.selected!.id)} onVerify={() => props.onVerify(props.selected!.id)} onReject={() => props.onReject(props.selected!.id)} onCorrective={() => props.onCorrective(props.selected!.id)} onCorrectiveAction={(responseId) => props.onCorrectiveAction(props.selected!.id, responseId)} onReinspection={() => props.onReinspection(props.selected!.id)} /> : <Card><CardContent className="py-12 text-center text-sm text-muted-foreground">Select an inspection to view checklist responses, evidence, observations and timeline.</CardContent></Card>}</div>;
}

function InspectionDetail({ inspection, categories, assets, responses, observations, evidence, certificates, requirements, onEvidence, onObservation, onComplete, onVerify, onReject, onCorrective, onCorrectiveAction, onReinspection }: { inspection: SafetyInspection; categories: SafetyCategory[]; assets: MaintenanceAsset[]; responses: SafetyInspectionResponse[]; observations: any[]; evidence: any[]; certificates: SafetyCertificate[]; requirements: SafetyInspectionTemplateEvidenceRequirement[]; onEvidence: () => void; onObservation: () => void; onComplete: () => void; onVerify: () => void; onReject: () => void; onCorrective: () => void; onCorrectiveAction: (responseId: string) => void; onReinspection: () => void }) {
  const care = useCare();
  ...
  return <Card><CardHeader><CardTitle>{inspection.inspectionNumber}</CardTitle><p className="text-sm text-muted-foreground">{categoryName(inspection.categoryId, categories)} - {assetName(inspection.assetId, assets) || "Home-level"} - {inspection.status.replaceAll("_", " ")}</p>{inspection.originalInspectionNumber && <p className="text-xs text-muted-foreground">Reinspection of {inspection.originalInspectionNumber}</p>}</CardHeader><CardContent className="space-y-4"><div className="grid gap-2 sm:grid-cols-3"><Stat label="Result" value={inspection.overallResult.replaceAll("_", " ")} /><Stat label="Verification" value={inspection.verificationStatus.replaceAll("_", " ")} /><Stat label="Evidence" value={evidence.filter((item) => item.active && !item.deletedAt).length} /></div>{evaluation.blockers.length > 0 && <div className="rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><div className="font-semibold">Completion checks</div>{evaluation.blockers.slice(0, 8).map((item) => <div key={item}>- {item}</div>)}</div>}<div className="space-y-3">{Object.entries(sections).map(([section, rows]) => <div key={section} className="rounded-lg border"><div className="border-b bg-muted/40 px-3 py-2 font-semibold">{section}</div>{rows.map((response) => <div key={response.id} className="grid gap-2 border-b p-3 text-sm last:border-0 md:grid-cols-[1fr_130px_220px]"><div><div className="font-medium">{response.questionLabelSnapshot}{response.mandatory && <span className="text-destructive"> *</span>}</div>{response.observation && <div className="text-xs text-muted-foreground">{response.observation}</div>}</div><select className="h-9 rounded-md border bg-background px-2" value={response.result} disabled={["COMPLETED", "FAILED"].includes(inspection.status)} onChange={(e) => care.updateSafetyInspectionResponse(response.id, { result: e.target.value as SafetyInspectionResponse["result"], responseValue: e.target.value })}>{["UNANSWERED", "PASS", "FAIL", "NOT_APPLICABLE", "INFORMATION_ONLY"].map((item) => <option key={item} value={item}>{item.replaceAll("_", " ")}</option>)}</select><div className="flex gap-1"><Button size="sm" variant="outline" onClick={() => care.addSafetyObservation(inspection.id, { responseId: response.id, description: response.observation || response.questionLabelSnapshot, severity: response.failureSeverity, correctiveActionRequired: response.correctiveActionRequired })}>Observation</Button>{response.result === "FAIL" && <Button size="sm" variant="outline" onClick={() => onCorrectiveAction(response.id)}>Corrective Action</Button>}</div></div>)}</div>)}</div><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={onEvidence}>Add Evidence</Button><Button variant="outline" onClick={onObservation}>Add Observation</Button><Button variant="outline" onClick={onCorrective}>Corrective Work Order</Button>{(inspection.overallResult === "FAIL" || inspection.status === "REJECTED") && <Button variant="outline" onClick={onReinspection}>Create Reinspection</Button>}<Button disabled={["COMPLETED", "FAILED"].includes(inspection.status)} onClick={onComplete}>Complete</Button><Button variant="outline" disabled={inspection.verificationStatus !== "PENDING"} onClick={onVerify}>Verify</Button><Button variant="outline" disabled={inspection.verificationStatus !== "PENDING"} onClick={onReject}>Reject</Button></div><InfoList title="Corrective links" rows={[...(inspection.linkedWorkOrderIds || []).map((id) => `Work Order: ${id}`), ...(inspection.linkedCorrectiveActionIds || []).map((id) => `Corrective Action: ${id}`)]} empty="No corrective records linked." /><InfoList title="Observations" rows={observations.map((item) => `${item.severity}: ${item.description}`)} empty="No observations recorded." /><InfoList title="Evidence" rows={evidence.filter((item) => item.active && !item.deletedAt).map((item) => `${item.evidenceType}: ${item.fileName}`)} empty="No evidence attached." /><InfoList title="Timeline" rows={timeline.map((item) => `${formatDateTime(item.at)} - ${item.summary}${item.reference ? ` - ${item.reference}` : ""}`)} empty="No timeline events." /></CardContent></Card>;
}
