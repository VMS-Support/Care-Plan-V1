...
export const HOUSEKEEPING_CLEANING_TYPES: HousekeepingCleaningType[] = [
  "ROUTINE",
  "DEEP",
  "ENHANCED",
  "TERMINAL",
...
export function validateHousekeepingTemplate(input: Partial<HousekeepingTemplate>) {
  ...
  if (!Number(input.estimatedDurationMinutes || 0) || Number(input.estimatedDurationMinutes) < 1)
    fieldErrors.estimatedDurationMinutes = "Estimated duration is required.";
  if (!input.defaultFrequencyType) fieldErrors.defaultFrequencyType = "Select a default frequency.";
  if (!Number(input.defaultFrequencyInterval || 0) || Number(input.defaultFrequencyInterval) < 1)
    fieldErrors.defaultFrequencyInterval = "Frequency interval must be at least 1.";
  if (input.effectiveTo && input.effectiveFrom && input.effectiveTo < input.effectiveFrom)
    fieldErrors.effectiveTo = "Effective to cannot be before effective from.";
  return { valid: Object.keys(fieldErrors).length === 0, fieldErrors };
}

export function validateHousekeepingSchedule(
  input: Partial<HousekeepingSchedule>,
  source: { templates: HousekeepingTemplate[]; assets: MaintenanceAsset[] },
) {
  const fieldErrors: Record<string, string> = {};
  const template = source.templates.find((item) => item.id === input.templateId);
  if (!input.homeId) fieldErrors.homeId = "Select a Home.";
  if (!template || !template.active || template.status !== "ACTIVE")
    fieldErrors.templateId = "Select an active cleaning template.";
  if (template?.homeId && input.homeId && template.homeId !== input.homeId)
    fieldErrors.templateId = "Template belongs to a different Home.";
  if (!input.locationId && !input.locationLabel && !input.roomId)
    fieldErrors.subject = "Select a room or location.";
  if (!input.frequencyType) fieldErrors.frequencyType = "Select a frequency.";
  if (!Number(input.frequencyInterval || 0) || Number(input.frequencyInterval) < 1)
    fieldErrors.frequencyInterval = "Frequency interval must be at least 1.";
  ...
  const asset = input.locationId
    ? source.assets.find((item) => item.id === input.locationId)
    : undefined;
  if (input.locationId && asset && input.homeId && asset.homeId !== input.homeId)
    fieldErrors.locationId = "Location belongs to a different Home.";
  return { valid: Object.keys(fieldErrors).length === 0, fieldErrors };
}

export function nextHousekeepingDueDate(
  date: string,
...
export function housekeepingDueStatus(
...
export function createHousekeepingResponsesFromTemplate(
  taskId: string,
  sections: HousekeepingTemplateSection[],
  items: HousekeepingTemplateItem[],
  user: string,
  now: string,
): HousekeepingTaskResponse[] {
  ...
  return items
    ...
    .map((item) => ({
      ...
      supervisorApprovalRequired: item.supervisorApprovalRequired,
      highRisk: item.highRisk,
      bulkCompletionEligible: item.bulkCompletionEligible,
      answeredBy: user,
      answeredAt: now,
    }));
}

export function markAllHousekeepingResponses(params: {
  template: HousekeepingTemplate;
  responses: HousekeepingTaskResponse[];
  user: string;
  now: string;
}) {
  if (params.template.highRisk || params.template.markAllCompleteDisabled)
    throw new Error(
      "This is a high-risk cleaning template. Each item must be confirmed individually.",
    );
  let completed = 0;
  const restricted: string[] = [];
  const responses = params.responses.map((response) => {
    const requiresIndividual =
      response.result === "FAIL" ||
      response.failureRequiresPhoto ||
      response.failureRequiresObservation ||
      response.failureRequiresException ||
      response.supervisorApprovalRequired ||
      response.highRisk ||
      response.bulkCompletionEligible === false;
    if (response.result !== "UNANSWERED") return response;
    if (requiresIndividual) {
      restricted.push(response.questionLabelSnapshot);
      return response;
    }
    completed += 1;
    return {
      ...
      responseValue: "Pass",
      answeredBy: params.user,
      answeredAt: params.now,
    };
  });
  return { responses, completed, restricted };
}
...
export function evaluateHousekeepingTask(params: {
  ...
  const unansweredMandatory = params.responses.filter(
    (item) => item.mandatory && item.result === "UNANSWERED",
  );
  const missingNaReasons = params.responses.filter(
    (item) =>
      item.result === "NOT_APPLICABLE" &&
      item.notApplicableReasonRequired &&
      !item.notApplicableReason?.trim(),
  );
  const missingFailureObservation = params.responses.filter(
    (item) =>
      item.result === "FAIL" && item.failureRequiresObservation && !item.observation?.trim(),
  );
  const missingFailurePhoto = params.responses.filter(
    (item) =>
      item.result === "FAIL" &&
      item.failureRequiresPhoto &&
      !activeEvidence.some(
        (evidence) => evidence.responseId === item.id && evidence.evidenceType === "PHOTO",
      ),
  );
  const missingFailureException = params.responses.filter(
    (item) =>
      item.result === "FAIL" &&
      item.failureRequiresException &&
      !params.exceptions.some(
        (exception) => exception.taskId === params.task.id && exception.status !== "CLOSED",
      ),
  );
  const missingTaskPhotos =
    params.task.photoEvidenceRequired &&
    activeEvidence.filter((item) => item.taskId === params.task.id && item.evidenceType === "PHOTO")
      .length < params.task.minimumPhotoCount;
  const openBlockingExceptions = params.exceptions.filter(
    (item) =>
      ["OPEN", "IN_REVIEW", "ACTION_REQUIRED"].includes(item.status) &&
      (item.severity === "HIGH" ||
        item.severity === "CRITICAL" ||
        item.requiresMaintenanceWorkOrder),
  );
  const blockers = [
    ...
    ...missingFailureObservation.map(
      (item) => `Record observation for failed item: ${item.questionLabelSnapshot}`,
    ),
    ...missingFailurePhoto.map(
      (item) => `Attach photo evidence for failed item: ${item.questionLabelSnapshot}`,
    ),
    ...missingFailureException.map(
      (item) => `Create exception for failed item: ${item.questionLabelSnapshot}`,
    ),
    ...
    ...(params.task.supervisorSignOffRequired && params.task.supervisorSignOffResult !== "APPROVED"
      ? ["Supervisor sign-off is required."]
      : []),
  ];
  const result =
    failedResponses.length || openBlockingExceptions.length
      ? "FAIL"
      : params.responses.some((item) => item.observation)
  ...
  return {
    ...
    nextStatus:
      result === "FAIL"
        ? ("FAILED" as const)
        : params.task.qualityInspectionRequired
          ? ("AWAITING_INSPECTION" as const)
          : ("COMPLETED" as const),
  };
}
...
export function roomReadinessBlockers(params: {
  ...
  if (params.readiness.cleaningRequired && !params.readiness.cleaningCompleted)
    blockers.push("Cleaning is not complete.");
  if (params.readiness.qualityInspectionRequired && !params.readiness.qualityInspectionPassed)
    blockers.push("Quality inspection has not passed.");
  ...
  if (
    params.exceptions.some(
      (item) =>
        item.roomId === params.readiness.roomId &&
        ["OPEN", "IN_REVIEW", "ACTION_REQUIRED"].includes(item.status),
    )
  )
    blockers.push("Open housekeeping exceptions remain.");
  if (
    params.workOrders.some(
      (item) =>
        String(item.roomId || "") === params.readiness.roomId &&
        ACTIVE_WORK_ORDER_STATUSES.includes(item.status),
    )
  )
    blockers.push("Open maintenance Work Order blocks readiness.");
  return blockers;
}
...
export function housekeepingDashboardMetrics(params: {
  ...
  const activeTasks = params.tasks.filter(
    (item) => !["COMPLETED", "CANCELLED", "SKIPPED"].includes(item.status),
  );
  const completedToday = params.tasks.filter((item) => item.completedAt?.startsWith(today));
  return {
    activeTemplates: params.templates.filter((item) => item.active && item.status === "ACTIVE")
      .length,
    activeSchedules: params.schedules.filter(
      (item) => item.active && !item.paused && !item.archivedAt,
    ).length,
    dueToday: activeTasks.filter((item) => item.dueDate === today).length,
    overdue: activeTasks.filter((item) => housekeepingDueStatus(item, params.today) === "OVERDUE")
      .length,
    ...
    roomBlocked: params.readiness.filter(
      (item) => !["READY", "OCCUPIED"].includes(item.readinessStatus),
    ).length,
    auditFailures: params.audits.filter(
      (item) => item.status === "FAILED" || item.result === "FAIL",
    ).length,
    completionRate: params.tasks.length
      ? Math.round(
          (params.tasks.filter((item) => item.status === "COMPLETED").length /
            params.tasks.length) *
            100,
        )
      : 0,
    byCleaningType: HOUSEKEEPING_CLEANING_TYPES.map((type) => ({
      ...
      completed: params.tasks.filter(
        (item) => item.cleaningType === type && item.status === "COMPLETED",
      ).length,
      failed: params.tasks.filter((item) => item.cleaningType === type && item.status === "FAILED")
        .length,
    })),
  };
}
